...
### 5. Dual Surfaces
- **FR-5.1 (Native Desktop)**: The Windows app MUST render a responsive floating PTT widget, Kanban board viewer, voice chat panel, trigger phrase config UI, and provider/STT/TTS/hotkey settings. *(The Kanban board viewer and voice chat panel are deferred for the current phase — see Decisions 33–34; implementations preserved, nav entries removed.)*
