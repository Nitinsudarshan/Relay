...
### Meeting Reminders: The App-Owned Notification Card Comes Back, on Meetings V2

**Type**: minor — new `meetings_v2::reminders` backend module and a restored `meeting-reminder` overlay window, rebuilt on V2's calendar and recorder rather than on the removed legacy meetings tree (`native/src-tauri/src/meetings_v2/reminders/*`, `native/src-tauri/src/overlay.rs`, `native/src/components/meetings_v2/MeetingReminderWindow.tsx`). See `docs/decisions.md` Decision 63.

#### Features

- **A reminder card with Record, Join and Snooze on it (`native/src/components/meetings_v2/MeetingReminderWindow.tsx`, `native/src-tauri/src/overlay.rs`)**: A floating `meeting-reminder` window in the top-right of the active monitor, created hidden at startup and reused for every reminder. **Record** starts a V2 recording titled from the meeting, brings the main window to the Meetings tab, and resolves every reminder for that meeting. **Join** opens the conferencing link in the default browser and re-arms the reminder a minute later, because joining a call is not recording it. **Snooze** offers 5, 10 or 15 minutes in place. The card never takes focus, is `content_protected` so it cannot appear in a screen share or recording, and auto-dismisses after 15 seconds — paused while the pointer is on it, and never resumed with less than 5 seconds left, so a card cannot vanish out from under the click on its way.
- **Three reasons a reminder exists (`native/src-tauri/src/meetings_v2/reminders/mod.rs`)**: `upcoming` five minutes before a scheduled calendar event; `unrecorded` a few minutes after a scheduled start while nothing is recording; `detected` for a Meet, Zoom, Teams or Webex call the calendar knows nothing about. Each is a separate switch under **Settings › Meetings › Meeting reminders**, defaulting on.
- **Conferencing-window detection (`native/src-tauri/src/meetings_v2/reminders/detection.rs`)**: Win32 `EnumWindows` over visible top-level windows, matching Zoom, Google Meet, Microsoft Teams and Webex and cleaning browser and app chrome out of the title. Windows only; every other platform reports nothing, which degrades to calendar-only reminders rather than to an error. Settings › Developer's **Check Window Detection** now renders what it found in place, with each window's provider and confidence, instead of an `alert()`.
- **The reminder loop (`native/src-tauri/src/meetings_v2/reminders/scheduler.rs`)**: One background task ticking every 15 seconds. Window detection runs every tick; the calendar is re-read every two minutes and cached, so reminder timing stays exact while Relay makes one Google call per two minutes rather than one per tick. With all three switches off it makes no calendar call at all.

#### Improvements

- **A reminder's key is not a session id (`native/src-tauri/src/commands.rs`)**: The field is `key`, prefixed `cal:` or `win:`, and `start_meeting_from_reminder` resolves it to a title and starts a new session. The removed implementation named it `meeting_id` and passed it to a command requiring an already-persisted meeting, so the popup's primary action returned `NOT_FOUND` for every reminder it ever showed (`docs/decisions.md` Decision 45, Broken #1).
- **The reminder queue is a queue (`native/src-tauri/src/meetings_v2/reminders/mod.rs`)**: One entry per `(key, kind)`, never a single overwritable slot, and the card to show is derived from the queue rather than stored beside it. The removed `ActiveReminderState` held one payload, so a second reminder erased the first before anybody had seen it (Decision 45, Broken #2). One card is on screen at a time, so two meetings starting in the same minute produce two cards in turn — the second waits in a backlog rather than stacking or being dropped, and is skipped if the user answers it from the Meetings list while the first is still up.
- **One path starts a recording (`native/src-tauri/src/commands.rs`)**: `start_meeting_v2` and `start_meeting_from_reminder` now share `start_meeting_session`, which raises the recording pill and clears any reminder card for that meeting. The Meetings list and the card can no longer disagree about whether a meeting has been dealt with (Decision 45, Refactor #1).
- **Evidence before an interruption (`native/src-tauri/src/meetings_v2/reminders/mod.rs`)**: A conferencing window whose title carries no topic (`Zoom Meeting`, `Google Meet Session`) must persist across two ticks before it earns a reminder; a window naming a real meeting is evidence enough on the first sighting. An `unrecorded` reminder fires only while the call is still on screen, so a meeting the user left is not reported as one they forgot to record, and a scheduled call never reports itself a second time as `detected`.
- **The time on the card is the real time (`native/src-tauri/src/meetings_v2/reminders/notification.rs`)**: "Starts in 4 minutes", "Starting now", "Started 6 minutes ago", computed from the event's own start. The removed implementation printed a fixed "Starts in 5 minutes" on a reminder that fired two minutes out.
- **Settings › Developer's reminder panel works again (`native/src/components/settings/DeveloperSettingsView.tsx`)**: `trigger_mock_meeting_reminder` and `debug_detect_conferencing_windows` had been left calling Tauri commands that no longer existed. Both are restored and typed, and the mock triggers go through the same `NotificationService::show` as a real reminder rather than taking a shortcut. The `notification_surface_mode` developer setting — stored but read by nothing since the removal — again selects the overlay, the OS toast, or both.

#### Security

- **A meeting title is untrusted text (`native/src-tauri/src/meetings_v2/reminders/notification.rs`)**: Titles come from calendar invitations and window titles. Control characters and Unicode bidirectional overrides (`U+202A`–`U+202E`, `U+2066`–`U+2069`) are stripped and the result clamped before it reaches a window, so a crafted title can neither misrepresent what the card says nor push its buttons off the card.
- **The card gets the smallest surface that works (`native/src-tauri/capabilities/meeting-reminder.json`)**: A dedicated capability granting the `meeting-reminder` window `core:default` and `core:event:default` only — no filesystem, shell, dialog or notification access. It reads its payload and reports its actions through Relay's own commands.
- **Join checks the scheme before the shell sees it (`native/src-tauri/src/commands.rs`)**: Only `http://` and `https://` links are opened, via `tauri-plugin-opener`. A calendar invitation must not be able to become a launcher.

#### Testing

- **The reminder queue and its gates (`native/src-tauri/src/meetings_v2/reminders/mod.rs`)**: 20 tests covering fire-once-per-transition, two simultaneous reminders both surviving and both being shown in turn, a reminder answered elsewhere not being shown late, no reminder waiting in the backlog twice, the lead window's edges, the unrecorded reminder's on-screen requirement, suppression while recording, generic-title graduation, a named call firing on the first sighting, a scheduled call not double-reporting as detected, a closed window dropping its reminder, snooze elapsing and returning, dismissal, per-kind switches, a finished meeting producing nothing, and a meeting already captured by an earlier recording not being reported as unrecorded.
- **Sanitization, the countdown, and the card's own wording (`native/src-tauri/src/meetings_v2/reminders/notification.rs`, `native/src-tauri/src/meetings_v2/reminders/detection.rs`)**: 14 tests covering bidi and control-character stripping, title clamping, the hover resume floor, the time label matching the real start, a Teams chat window not being read as a call, browser chrome stripping, and detection returning empty rather than failing off Windows.
- **What a person sees and presses (`native/src/components/meetings_v2/MeetingReminderWindow.test.tsx`)**: 12 tests asserting the command each button sends, that no Join button is offered for a meeting with no link, that snooze durations can be backed out of without answering the reminder, that hover is reported to the backend, and that the card names why it is on screen. 34 new backend tests and 12 new frontend tests, with 1,262 backend and 479 frontend tests passing.

#### Superseded in this release

- **The OS-toast reminder pipeline is replaced by the card (`native/src-tauri/src/meetings_v2/reminders.rs`, `reminder_engine.rs`, `detection.rs`, `native/src/App.tsx`)**: An earlier v0.41.0 change shipped this feature as native Windows toasts carrying `▶ Record`, `◷ Snooze 5m/15m` and `Dismiss` buttons, wired through `.action_type_id()`, `registerActionTypes` and `onAction`. None of that reaches a Windows desktop. In `tauri-plugin-notification` 2.3.3 — the version this crate pins — `init()` registers only `notify`, `request_permission` and `is_permission_granted`, so `registerActionTypes` has no desktop command to call; `desktop.rs`'s `show()` maps only title, body, icon and sound, dropping `action_type_id`; `register_action_types` exists solely in `mobile.rs`; and `onAction` waits on an `actionPerformed` event desktop never emits. The toast therefore appeared with no buttons and nothing routed — the same wall documented in v0.10.0, which is why Decision 46 was reversed the first time. Those four modules and the `App.tsx` notification wiring are removed in favour of the card, which can carry actions because Relay owns the window. The demoted display-only toast is kept, selectable via Settings › Developer's notification surface mode.
- **Kept from it (`native/src-tauri/src/meetings_v2/reminders/mod.rs`)**: its unrecorded gate was the better one and is ported. A reminder now asks whether any recording — running *or* already finished — covers the meeting's time window, rather than only whether something is recording at this instant, so a meeting captured and stopped early is no longer reported as unrecorded.
