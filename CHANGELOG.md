...
- **The reminder queue and its gates (`native/src-tauri/src/meetings_v2/reminders/mod.rs`)**: 18 tests covering fire-once-per-transition, two simultaneous reminders both surviving and both being shown in turn, a reminder answered elsewhere not being shown late, no reminder waiting in the backlog twice, the lead window's edges, the unrecorded reminder's on-screen requirement, suppression while recording, generic-title graduation, a named call firing on the first sighting, a scheduled call not double-reporting as detected, a closed window dropping its reminder, snooze elapsing and returning, dismissal, per-kind switches, and a finished meeting producing nothing.
