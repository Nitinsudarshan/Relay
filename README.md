...
- **Capture** — Saves the web page or AI conversation you are looking at into your vault as structured text, not a screenshot: turn-by-turn conversations from ChatGPT and Claude, repositories, issues and pull requests from GitHub, and article text, tables, code and metadata from anything else. It reads more than the screen: Relay scrolls a long conversation from its start, waits for content that loads as you go, and opens sections that are genuinely collapsed — then puts your scroll position back. Files, generated files and images are recorded with their details and marked plainly as not downloaded. Every capture records where it came from and how much of the page was actually read, with the counts behind that claim. Captured pages are stored as external source material, never as instructions to Relay's AI. A browser extension reads the page under `activeTab` (granted only when you press its shortcut) and posts it to Relay over loopback; nothing goes to a server. See [`docs/capture.md`](docs/capture.md).
...
```bash
# Rust backend — 825 tests (+4 ignored benchmarks)
cd native/src-tauri && cargo clippy --all-targets -- -D warnings && cargo test

# Native frontend — 221 tests
