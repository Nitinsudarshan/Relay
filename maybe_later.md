...
- **Original Context**:
  - The pre-V2 meetings plan (removed once V2 shipped; see CHANGELOG v0.14.0) specified a detection and reminder architecture that the V2 rebuild only partly implements. **Now built** (see `docs/decisions.md` Decision 63): calendar-proximity and conferencing-window triggers, feeding a reminder card with Record, Join and Snooze on it. **Still outstanding**: a sustained-audio trigger, auto-start without a prompt, and auto-end — a meeting is still stopped by hand, and started by hand or by one press on the reminder.
...
- **Concept & Implementation Blueprint**:
  - **Detection**: one service normalizing calendar, process, sustained-audio, and manual signals into `meeting-detected` / `meeting-ended` events, deduplicated by a fingerprint that is explicitly not identity. Detection must never start recording directly — it prompts, and the session manager decides. *Partly built:* `meetings_v2::reminders` does exactly this for the calendar and window-title signals, keyed by a `cal:`/`win:` string that is deliberately not a session id; a sustained-audio signal and `meeting-ended` are what remain.
